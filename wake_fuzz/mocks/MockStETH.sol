...
contract MockStETH is ERC20 {
    ...
    function mock_burnExternalShares(uint256 _amountOfShares) external {
        require(_amountOfShares != 0, "BURN_ZERO_AMOUNT_OF_SHARES");
        uint256 pooledEther = getPooledEthByShares(_amountOfShares);

        console.log("updating shares");
        console.log("recipent shares", shares[msg.sender]);
        shares[msg.sender] -= _amountOfShares;
        console.log("updated shares");
        totalShares -= _amountOfShares;
        console.log("updated total shares");
        totalPooledEth -= pooledEther;
    }
    ...
    /**
     * @notice Simulate natural stETH yield: increase pooled ETH while keeping shares constant
     * @dev Mirrors Lido oracle reports where rewards make each share worth more
     */
    function mock_accrueYield(uint256 _ethEarned) external {
        require(_ethEarned != 0, "YIELD_ZERO");
        totalPooledEth += _ethEarned;
    }

    /**
     * @notice Simulate losses/slashing by reducing pooled ETH at a fixed share count
     * @dev Lowers the share rate; reverts if loss exceeds pooled ETH
     */
    function mock_applyLoss(uint256 _ethLost) external {
        require(_ethLost != 0, "LOSS_ZERO");
        require(_ethLost <= totalPooledEth, "LOSS_EXCEEDS_POOL");
        totalPooledEth -= _ethLost;
    }
}
