...
contract MockVaultFactory is IVaultFactory {
    ...
    constructor(address _vaultHub, address _wsteth) { /// need to create before hand for dependency so moving deploy of wsteth out.
        VAULT_HUB = _vaultHub;
    ...
    function createVaultWithDashboardWithoutConnectingToVaultHub(
        ...
        emit DashboardCreated(address(dashboard), address(vault), _admin);
        return (vault, dashboard);
    }
}
