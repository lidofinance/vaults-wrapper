...
contract MockDashboard is AccessControlEnumerable {
    ...
    constructor(address _steth, address _wsteth, address _vaultHub, address _stakingVault, address _admin) {
        ...
        VAULT_HUB.mock_setConnectionParameters(VAULT, 10_00, 8_00); // 10% reserve, 9.75% forced rebalance # change for test.
    }
    ...
    function burnWstETH(uint256 amount) external {
        ...
        STETH.transferShares(address(VAULT_HUB), unwrappedShares); // smaller than amount
        VAULT_HUB.burnShares(VAULT, unwrappedShares);// it was wrongly set amount here.
    }
