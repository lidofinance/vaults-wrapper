...
contract WithdrawalQueueHappyPathTest is Test, SetupWithdrawalQueue {
    ...
    function test_WithdrawalQueue_HappyPath() public {
        ...
        pool.requestWithdrawal(initialStv, 0, 0, address(this));

        // Request 1: Withdraw part of the position
        uint256 firstWithdrawStv = initialStv / 5;
        uint256 firstRequestId = pool.requestWithdrawal(firstWithdrawStv, 0, 0, address(this));
        ...
        pool.claimWithdrawal(firstRequestId, address(this));
        ...
        // Request 2: Try to request with burning but without allowance - should fail
        uint256 secondSharesToBurn = mintedShares / 3;
        uint256 secondWithdrawStv = initialStv / 5;
        vm.expectRevert("Not enough allowance");
        pool.requestWithdrawal(secondWithdrawStv, secondSharesToBurn, 0, address(this));

        // Request 2: Approve and request withdrawal with burning
        steth.approve(address(pool), steth.getPooledEthByShares(secondSharesToBurn));
        uint256 secondRequestId = pool.requestWithdrawal(secondWithdrawStv, secondSharesToBurn, 0, address(this));
        ...
        uint256 thirdRequestId = pool.requestWithdrawal(thirdWithdrawStv, 0, thirdSharesToRebalance, address(this));
        ...
        uint256[] memory hints = withdrawalQueue.findCheckpointHints(secondRequestIds, 2, lastCheckpointIndex);
        ...
        pool.claimWithdrawals(secondRequestIds, hints, address(this));
