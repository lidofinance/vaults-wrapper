...
contract WithdrawalQueueTest is Test {
    ...
    event WithdrawalsFinalized(uint256 firstRequestId, uint256 lastRequestId, uint256 totalAssets, uint256 totalShares, uint256 shareRate);
    ...
    function test_NormalWithdrawalFlow() public {
        ...
        withdrawalQueue.finalize(2, totalAssetsToFinalize, wrapper.calculateShareRate()); // Finalize both requests
        vm.stopPrank();
    ...
    function test_WithdrawalWithSlashing() public {
        ...
        console.log("Vault totalValue:", vaultHub.totalValue(address(stakingVault)));
        console.log("Wrapper totalAssets:", wrapper.totalAssets());
        console.log("Wrapper totalSupply:", wrapper.totalSupply());
        ...
        uint256 newTotalValue = vaultHub.totalValue(address(stakingVault)) * 80 / 100; // 20% loss
        ...
        withdrawalQueue.finalize(3, availableAfterSlashing, wrapper.calculateShareRate()); // Finalize all requests
        vm.stopPrank();
        
        // 5. Check checkpoint with discounted share rate
        WithdrawalQueue.Checkpoint memory checkpoint = withdrawalQueue.getCheckpoint(1);
        ...
        uint256 expectedUser1Balance = 100 ether - 10 ether + (5 ether * 80 / 100); // 94 ETH
        uint256 expectedUser2Balance = 100 ether - 20 ether + (8 ether * 80 / 100); // 86.4 ETH
        uint256 expectedUser3Balance = 100 ether - 15 ether + (6 ether * 80 / 100); // 89.8 ETH
        
        console.log("\nFinal balances:");
        console.log("User1 expected: %s ETH, actual: %s ETH", expectedUser1Balance, user1.balance);
        console.log("User2 expected: %s ETH, actual: %s ETH", expectedUser2Balance, user2.balance);
        console.log("User3 expected: %s ETH, actual: %s ETH", expectedUser3Balance, user3.balance);
    ...
    function test_PartialFinalization() public {
        ...
        withdrawalQueue.finalize(2, totalRequested, wrapper.calculateShareRate()); // Only finalize request1 and request2
        vm.stopPrank();
        ...
        withdrawalQueue.finalize(3, totalRequested, wrapper.calculateShareRate());
        vm.stopPrank();
    ...
    function test_FIFOOrder() public {
        ...
        vm.startPrank(nodeOperator);
        uint256 totalRequested = 5 ether + 8 ether; // 13 ETH
        withdrawalQueue.finalize(1, totalRequested, wrapper.calculateShareRate()); // First finalize request1
        withdrawalQueue.finalize(2, totalRequested, wrapper.calculateShareRate()); // Then finalize request2
        vm.stopPrank();
