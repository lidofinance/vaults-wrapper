...
contract WrapperB is WrapperBase {
    ...
    function depositETH(address _receiver, address _referral) public payable virtual override returns (uint256 stvShares) {
        ...
        return depositETH(_receiver, _referral, targetStethShares);
    }

    function depositETH(address _receiver, address _referral, uint256 _stethSharesToMint) public payable returns (uint256 stvShares) {
     ...
     */
    function stethSharesForWithdrawal(address _address, uint256 _stvShares) public view returns (uint256 stethShares) {
        ...
        // TODO: Ceil or Floor?
        stethShares = Math.mulDiv(_stvShares, _getStethShares(_address), balance, Math.Rounding.Floor);
    }
    ...
    function requestWithdrawal(uint256 _stvShares) public virtual returns (WithdrawalRequest memory) {
        uint256 withdrawalQueueRequestId = _requestWithdrawalQueue(msg.sender, msg.sender, _stvShares);

        WrapperBaseStorage storage $ = _getWrapperBaseStorage();
        uint256 requestId = $.withdrawalRequests.length;
        WithdrawalRequest memory request = WithdrawalRequest({
            requestId: withdrawalQueueRequestId,
            requestType: WithdrawalType.WITHDRAWAL_QUEUE,
            owner: msg.sender
        });
    ...
    function _requestWithdrawalQueue(address _owner, address _receiver, uint256 _stvShares) internal returns (uint256 requestId) {
        if (_stvShares == 0) revert WrapperBase.ZeroStvShares();

        WithdrawalQueue withdrawalQueue = withdrawalQueue();

        uint256 stethShares = stethSharesForWithdrawal(_owner, _stvShares);

        _transfer(_owner, address(this), _stvShares);
        ...
        requestId = withdrawalQueue.requestWithdrawal(_stvShares, _receiver);
    }
    ...
    function _getPartCorrespondingToStvShares(uint256 _stvShares, uint256 _assets) internal view returns (uint256 assets) {
        assets = Math.mulDiv(_stvShares, _assets, totalSupply(), Math.Rounding.Floor);
    }
