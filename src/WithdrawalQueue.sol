...
contract WithdrawalQueue is AccessControlEnumerable, Pausable {
    ...
    function calculateFinalizationBatches(
        ...
        if (_state.batchesLength == 0) {
            ...
            (prevRequestShareRate, , ) = _calcBatch(
                requests[lastHandledRequestId - 1],
                prevRequest
            );
        }
    ...
    function requestWithdrawal(
        ...
        if (!vaultHub.isReportFresh(WRAPPER.STAKING_VAULT()))
            revert ReportStale();
    ...
    function finalize(
        ...
        require(
            _lastRequestIdToFinalize > getLastFinalizedRequestId(),
            "Invalid request ID"
        );
        require(
            _lastRequestIdToFinalize <= getLastRequestId(),
            "Request not found"
        );
        ...
        WithdrawalRequest memory lastFinalized = requests[
            lastFinalizedRequestId_
        ];
        WithdrawalRequest memory toFinalize = requests[
            _lastRequestIdToFinalize
        ];

        uint256 totalAssetsToFinalize = toFinalize.cumulativeAssets -
            lastFinalized.cumulativeAssets;
        uint256 totalSharesToFinalize = toFinalize.cumulativeShares -
            lastFinalized.cumulativeShares;
        ...
        for (
            uint256 i = firstRequestIdToFinalize;
            i <= _lastRequestIdToFinalize;
            i++
        ) {
    ...
    function claimWithdrawal(uint256 _requestId) external {
        uint256 checkpoint = _findCheckpointHint(
            _requestId,
            1,
    ...
    function _findCheckpointHint(
        ...
        if (_start == 0 || _end > lastCheckpointIndex_)
    ...
    function getWithdrawalStatus(
        uint256 _requestId
    ) external view returns (WithdrawalRequestStatus memory status) {
    ...
    function getClaimableEther(
        ...
        for (uint256 i = 0; i < _requestIds.length; ++i) {
            claimableEthValues[i] = _getClaimableEther(
                _requestIds[i],
                _hints[i]
            );
        }
    ...
    function unfinalizedAssets() external view returns (uint256) {
        return
            requests[getLastRequestId()].cumulativeAssets -
            requests[getLastFinalizedRequestId()].cumulativeAssets;
    }

    function unfinalizedShares() external view returns (uint256) {
        return
            requests[getLastRequestId()].cumulativeShares -
            requests[getLastFinalizedRequestId()].cumulativeShares;
    }
    ...
    function _checkWithdrawalRequestAmount(uint256 _amount) internal pure {
        ...
        if (_amount > MAX_WITHDRAWAL_AMOUNT)
    ...
    {
        assets =
            _endRequest.cumulativeAssets -
            _preStartRequest.cumulativeAssets;
        shares =
    ...
    function _getStatus(
        uint256 _requestId
    ) internal view returns (WithdrawalRequestStatus memory) {
        if (_requestId == 0 || _requestId > getLastRequestId())
            revert InvalidRequestId(_requestId);
        ...
        return
            WithdrawalRequestStatus({
                amountOfAssets: request.cumulativeAssets -
                    previousRequest.cumulativeAssets,
                amountOfShares: request.cumulativeShares -
                    previousRequest.cumulativeShares,
                owner: request.owner,
